...
srdbManagable = srdbData %>% select(Longitude, Latitude, Study_midyear, Ecosystem_type, Manipulation, Rs_annual, RC_annual, Soil_drainage)
...
# raster::as.data.frame(map_data, xy = TRUE) %>%
...
# 
# #temp mean
# tmeanGlobal = as.data.frame(mat_data, xy = TRUE) %>% drop_na()
...
srdbFinal %>% ggplot(aes(Longitude, Latitude, color = map_srdb)) + geom_point(size = srdbFinal$RC_annual * 10) + scale_color_gradient(low = "skyblue", high = "blue4")  + coord_cartesian(xlim = c(-180, 180), ylim = c(-90, 90))

#RC_annual vs temperature with size by precip and color also by temp
srdbFinal %>% ggplot(aes(mat_srdb, RC_annual, color = mat_srdb)) + scale_color_gradient(low = "yellow1", high = "red1") + geom_point(size = srdbFinal$map_srdb * .01)
...
srdbFinal %>% 
  ggplot(aes(mat_srdb, map_srdb)) + 
  geom_point(aes(color = mat_srdb)) 

# check 
#latitude vs mat (helpful for future presentation?)
srdbFinal %>% ggplot(aes(Latitude, mat)) + geom_point()
