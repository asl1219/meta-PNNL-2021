...
srdbCoords = srdbData %>% dplyr::filter(RC_annual != "N/A") %>% dplyr::select(Longitude, Latitude, RC_annual)
...
srdbMapMat %>% ggplot(aes(Longitude, Latitude, color = map)) + geom_point(size = srdbMapMat$RC_annual * 10) + scale_color_gradient(low = "skyblue", high = "blue4")  + coord_cartesian(xlim = c(-180, 180), ylim = c(-90, 90))

#RC_annual vs temperature with size by precip and color also by temp
srdbMapMat %>% ggplot(aes(mat, RC_annual, color = mat)) + scale_color_gradient(low = "yellow1", high = "red1") + geom_point(size = srdbMapMat$map * .01)
...
#srdbMapMat %>% ggplot(aes(mat, map)) + geom_line(size = srdbMapMat$RC_annual * 2)

# check 
#latitude vs mat (helpful for future presentation?)
srdbMapMat %>% ggplot(aes(Latitude, mat)) + geom_point()
