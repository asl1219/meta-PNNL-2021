...
```{r}
#n/a's not working so remove?
srdbFinalRF = srdbFinal
...
#size of testing set
data_set_size = floor(nrow(srdbFinalRF)/2)
...
RcPrediction = predict(object = RcRandFor, newdata = validationSet[ , -7], type = "response")
RcConfusion = data.frame(observed = validationSet[ , 7], predicted = RcPrediction)
```
